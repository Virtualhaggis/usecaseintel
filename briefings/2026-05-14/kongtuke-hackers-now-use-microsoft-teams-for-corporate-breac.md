# [CRIT] KongTuke hackers now use Microsoft Teams for corporate breaches

**Source:** BleepingComputer
**Published:** 2026-05-14
**Article:** https://www.bleepingcomputer.com/news/security/kongtuke-hackers-now-use-microsoft-teams-for-corporate-breaches/
...
Initial access brokers (IAB) like KongTuke typically s…
...
**Splunk SPL (CIM):**
```spl
``` Article-specific bespoke detection — KongTuke hackers now use Microsoft Teams for corporate breaches ```
| tstats `summariesonly` count earliest(_time) AS firstTime latest(_time) AS lastTime
    from datamodel=Endpoint.Processes
    where (Processes.process_name IN ("pmanager.py"))
...
| tstats `summariesonly` count
    ...
    where Filesystem.action IN ("created","modified")
      AND (Filesystem.file_name IN ("pmanager.py"))
