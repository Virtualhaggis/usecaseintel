# [CRIT] Hackers Used Claude AI to Attack on Water and Drainage Utility Systems

**Source:** Cyber Security News
**Published:** 2026-05-07
**Article:** https://cybersecuritynews.com/hackers-used-claude-ai-to-attack/
...
**Splunk SPL (CIM):**
```spl
``` Article-specific bespoke detection — Hackers Used Claude AI to Attack on Water and Drainage Utility Systems ```
| tstats `summariesonly` count earliest(_time) AS firstTime latest(_time) AS lastTime
    from datamodel=Endpoint.Processes
    where (Processes.process_name IN ("backuposint.py"))
...
| tstats `summariesonly` count
    ...
    where Filesystem.action IN ("created","modified")
      AND (Filesystem.file_name IN ("backuposint.py"))
...
DeviceProcessEvents
| where Timestamp > ago(30d)
| where (FileName in~ ("backuposint.py"))
