# [CRIT] Hackers Use PlugX-Like DLL Sideloading Chain in Fake Claude Malware Campaign

**Source:** Cyber Security News
**Published:** 2026-05-11
**Article:** https://cybersecuritynews.com/hackers-use-plugx-like-dll-sideloading-chain/
...
- **Domain (defanged):** `update-treix.com`
...
**Splunk SPL (CIM):**
```spl
``` Article-specific bespoke detection — Hackers Use PlugX-Like DLL Sideloading Chain in Fake Claude Malware Campaign ```
| tstats `summariesonly` count earliest(_time) AS firstTime latest(_time) AS lastTime
    from datamodel=Endpoint.Processes
    where (Processes.process_name IN ("claude.msi","novupdate.exe","avk.dll","goddtv.msi","mpcopyaccelerator.exe","mpclient.dll"))
...
| tstats `summariesonly` count
    ...
    where Filesystem.action IN ("created","modified")
      AND (Filesystem.file_name IN ("claude.msi","novupdate.exe","avk.dll","goddtv.msi","mpcopyaccelerator.exe","mpclient.dll"))
...
DeviceProcessEvents
| where Timestamp > ago(30d)
| where (FileName in~ ("claude.msi", "novupdate.exe", "avk.dll", "goddtv.msi", "mpcopyaccelerator.exe", "mpclient.dll"))
...
- **Network connections to article IPs / domains** ([template](../_TEMPLATES.md#network-ioc)) — phase: **c2**, confidence: **High**
  - IP / domain IOC(s): `claude-pro.com`, `license.claude-pro.com`, `vertextrust-advisors.com`, `gouvvbo.top`, `update-treix.com`, `update-crowdstrike.com`, `update-sentinelone.com`
