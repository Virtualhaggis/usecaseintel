# [CRIT] TCLBANKER Banking Trojan Targets Financial Platforms via WhatsApp and Outlook Worms

**Source:** The Hacker News
**Published:** 2026-05-08
**Article:** https://thehackernews.com/2026/05/tclbanker-banking-trojan-targets.html
...
The activity is being tracked by Elastic Security Labs under the moniker REF3076 . The malware family is assessed to be a major update of the Maverick , which is known to leve…
...
`UC_135_11` · phase: **install** · confidence: **High**
...
**Splunk SPL (CIM):**
```spl
``` Article-specific bespoke detection — TCLBANKER Banking Trojan Targets Financial Platforms via WhatsApp and Outlook Wo ```
| tstats `summariesonly` count earliest(_time) AS firstTime latest(_time) AS lastTime
    from datamodel=Endpoint.Processes
    where (Processes.process_name IN ("screen_retriever_plugin.dll","logiaipromptbuilder.exe","tclloader.exe","ntdll.dll"))
...
| tstats `summariesonly` count
    ...
    where Filesystem.action IN ("created","modified")
      AND (Filesystem.file_name IN ("screen_retriever_plugin.dll","logiaipromptbuilder.exe","tclloader.exe","ntdll.dll"))
...
DeviceProcessEvents
| where Timestamp > ago(30d)
| where (FileName in~ ("screen_retriever_plugin.dll", "logiaipromptbuilder.exe", "tclloader.exe", "ntdll.dll"))
