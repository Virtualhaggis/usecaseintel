# [HIGH] Police seize “First VPN” service used in ransomware, data theft attacks

**Source:** BleepingComputer
**Published:** 2026-05-21
**Article:** https://www.bleepingcomputer.com/news/security/police-seize-first-vpn-service-used-in-ransomware-data-theft-attacks/
...
`UC_58_4` · phase: **c2** · confidence: **High**

**Splunk SPL (CIM):**
```spl
| tstats `summariesonly` count min(_time) as firstTime max(_time) as lastTime values(DNS.src) as src values(DNS.dest) as dest values(DNS.query_type) as query_type from datamodel=Network_Resolution.DNS where DNS.query IN ("1vpns.com","*.1vpns.com","1vpns.net","*.1vpns.net","1vpns.org","*.1vpns.org") by DNS.query DNS.src host _time span=1h | `drop_dm_object_name(DNS)` | sort 0 - _time
```

**Defender KQL:**
```kql
let FirstVpnDomains = dynamic(["1vpns.com","1vpns.net","1vpns.org"]);
DeviceNetworkEvents
| where Timestamp > ago(180d)
| where isnotempty(RemoteUrl)
| where RemoteUrl has_any (FirstVpnDomains)
| summarize FirstSeen = min(Timestamp), LastSeen = max(Timestamp), Hits = count(),
            AnyProcess = any(InitiatingProcessFileName),
            AnyCmd = any(InitiatingProcessCommandLine),
            AnyRemoteIP = any(RemoteIP)
