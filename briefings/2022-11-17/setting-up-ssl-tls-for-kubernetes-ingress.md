# [HIGH] Setting up SSL/TLS for Kubernetes Ingress

**Source:** Snyk
**Published:** 2022-11-17
**Article:** https://snyk.io/blog/setting-up-ssl-tls-for-kubernetes-ingress/

## Threat Profile

Snyk Blog In this article
Written by Peter De Tender 
November 17, 2022
0 mins read Today, web and mobile applications and API-based microservice endpoints are becoming the default. These applications are reachable through the HTTP web protocol. The encryption provided by a Secured Socket Layer or Transport Layer Security (SSL/TLS) is a must to secure the communication between client and server and across API back-ends.
SSL/TLS are certificate-based encryption mechanisms. SSL has been the standa…
...
`UC_1815_2` · phase: **install** · confidence: **High**

**Splunk SPL (CIM):**
```spl
``` Article-specific bespoke detection — Setting up SSL/TLS for Kubernetes Ingress ```
| tstats `summariesonly` count earliest(_time) AS firstTime latest(_time) AS lastTime
    from datamodel=Endpoint.Processes
    where (Processes.process_path="*C:\Windows\System32\Drivers\Etc\Hosts*")
...
| tstats `summariesonly` count
    ...
    where Filesystem.action IN ("created","modified")
      AND (Filesystem.file_path="*C:\Windows\System32\Drivers\Etc\Hosts*" OR Filesystem.file_path="*/etc/hosts*")
...
| where (FolderPath has_any ("C:\Windows\System32\Drivers\Etc\Hosts"))
...
| where (FolderPath has_any ("C:\Windows\System32\Drivers\Etc\Hosts", "/etc/hosts"))
