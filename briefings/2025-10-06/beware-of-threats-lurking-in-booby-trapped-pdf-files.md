# [CRIT] Beware of threats lurking in booby-trapped PDF files

**Source:** ESET WeLiveSecurity
**Published:** 2025-10-06
**Article:** https://www.welivesecurity.com/en/malware/threats-lurking-pdf-files/
...
However, this all is also partly what makes PDFs the perfect disguise for all manner of threats. At first glance, PDF files seem…
...
**Splunk SPL (CIM):**
```spl
``` Article-specific bespoke detection — Beware of threats lurking in booby-trapped PDF files ```
| tstats `summariesonly` count earliest(_time) AS firstTime latest(_time) AS lastTime
    from datamodel=Endpoint.Processes
    where (Processes.process_name IN ("invoice.pdf.exe","document.pdf.scr"))
...
| tstats `summariesonly` count
    ...
    where Filesystem.action IN ("created","modified")
      AND (Filesystem.file_name IN ("invoice.pdf.exe","document.pdf.scr"))
...
| where (FileName in~ ("invoice.pdf.exe", "document.pdf.scr"))
