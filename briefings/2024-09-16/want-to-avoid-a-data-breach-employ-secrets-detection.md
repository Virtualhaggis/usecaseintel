# [HIGH] Want to avoid a data breach? Employ secrets detection

**Source:** Snyk
**Published:** 2024-09-16
**Article:** https://snyk.io/blog/want-to-avoid-data-breach-employ-secrets-detection/
...
`UC_1114_1` · phase: **exploit** · confidence: **High**

**Splunk SPL (CIM):**
```spl
``` Article-specific bespoke detection — Want to avoid a data breach? Employ secrets detection ```
| tstats `summariesonly` count earliest(_time) AS firstTime latest(_time) AS lastTime
    from datamodel=Endpoint.Processes
    where (Processes.process_name IN ("config.py"))
...
| tstats `summariesonly` count
    ...
    where Filesystem.action IN ("created","modified")
      AND (Filesystem.file_name IN ("config.py"))
