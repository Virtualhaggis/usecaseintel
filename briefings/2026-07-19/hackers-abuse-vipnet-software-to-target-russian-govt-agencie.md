# [MED] Hackers abuse ViPNet software to target Russian govt agencies

**Source:** BleepingComputer
**Published:** 2026-07-19
**Article:** https://www.bleepingcomputer.com/news/security/hackers-abuse-vipnet-software-to-target-russian-govt-agencies/
...
**Splunk SPL (CIM):**
```spl
``` Article-specific bespoke detection — Hackers abuse ViPNet software to target Russian govt agencies ```
| tstats `summariesonly` count earliest(_time) AS firstTime latest(_time) AS lastTime
    from datamodel=Endpoint.Processes
    where (Processes.process_name IN ("wtsapi32.dll","itcsrvup64.exe"))
...
| tstats `summariesonly` count
    ...
    where Filesystem.action IN ("created","modified")
      AND (Filesystem.file_name IN ("wtsapi32.dll","itcsrvup64.exe"))
...
| where (FileName in~ ("wtsapi32.dll", "itcsrvup64.exe"))
