# [CRIT] StealC and Amadey: Breaking down infostealers and the cybercrime services that deliver them

**Source:** Microsoft Security Blog, ESET WeLiveSecurity
**Published:** 2026-06-24
**Article:** https://www.microsoft.com/en-us/security/blog/2026/06/24/stealc-and-amadey-breaking-down-infostealers-and-the-cybercrime-services-that-deliver-them/
...
Threat intelligence 
Infostealers continue to be some of the most pervasive and impactful threats across the cybercrime ecosystem. They play a central role in intrusions, silently harvesting passwords, cookies, and session tokens befor…
...
`UC_98_19` · phase: **install** · confidence: **Medium** · AI-generated for this article
...
`UC_98_16` · phase: **exploit** · confidence: **High**

**Splunk SPL (CIM):**
```spl
``` Article-specific bespoke detection — StealC and Amadey: Breaking down infostealers and the cybercrime services that d ```
| tstats `summariesonly` count earliest(_time) AS firstTime latest(_time) AS lastTime
    from datamodel=Endpoint.Processes
    where (Processes.process_name IN ("nudwee.exe","cred.dll","clip.dll"))
...
| tstats `summariesonly` count
    ...
    where Filesystem.action IN ("created","modified")
      AND (Filesystem.file_name IN ("nudwee.exe","cred.dll","clip.dll"))
...
DeviceProcessEvents
| where Timestamp > ago(30d)
| where (FileName in~ ("nudwee.exe", "cred.dll", "clip.dll"))
