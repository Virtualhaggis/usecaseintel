# [CRIT] Injective Labs GitHub Compromise Pushes Wallet-Key-Stealing npm Packages

**Source:** The Hacker News
**Published:** 2026-07-10
**Article:** https://thehackernews.com/2026/07/injective-labs-github-compromise-pushes.html
...
The compromised version, @injectivelabs/sdk-ts@1.20.21 , came embedded with fake telemetry functionality that exfiltrated data from cry…
...
`UC_115_10` · phase: **c2** · confidence: **High** · AI-generated for this article

**Splunk SPL (CIM):**
```spl
| tstats summariesonly=t count min(_time) as firstTime max(_time) as lastTime from datamodel=Network_Resolution.DNS where DNS.query="testnet.archival.chain.grpc-web.injective.network" by DNS.src DNS.dest DNS.query DNS.answer | `drop_dm_object_name("DNS")` | convert ctime(firstTime) ctime(lastTime) | sort - lastTime
```

**Defender KQL:**
```kql
let c2 = "testnet.archival.chain.grpc-web.injective.network";
union
(DeviceNetworkEvents
| where Timestamp > ago(30d)
| where RemoteUrl has c2
| project Timestamp, DeviceName, InitiatingProcessFileName, InitiatingProcessCommandLine, RemoteUrl, RemoteIP, RemotePort),
(DeviceEvents
| where Timestamp > ago(30d)
| where ActionType == "DnsQueryResponse"
| where RemoteUrl has c2
| project Timestamp, DeviceName, InitiatingProcessFileName, InitiatingProcessCommandLine, RemoteUrl, RemoteIP=RemoteIP, RemotePort=int(null))
| order by Timestamp desc
```

### Trojanized @injectivelabs/sdk-ts 1.20.21 artifact by SHA256 on developer/CI endpoints

`UC_115_11` · phase: **install** · confidence: **Medium** · AI-generated for this article

**Splunk SPL (CIM):**
```spl
| tstats summariesonly=t count min(_time) as firstTime max(_time) as lastTime from datamodel=Endpoint.Filesystem where (Filesystem.file_hash="103c4e6181151c1bcfedc41506cd1815458c38375d08a8fcd9981dbe0b965ce0" OR Filesystem.file_hash="9a59eb454f3ca3fe91214136ee5edd417cc47a80e6f169b52099d6561944baf9") by Filesystem.dest Filesystem.file_name Filesystem.file_path Filesystem.file_hash | `drop_dm_object_name("Filesystem")` | convert ctime(firstTime) ctime(lastTime) | sort - lastTime
```
...
| where FileName in~ ("npm.exe","node.exe","yarn.exe","pnpm.exe","npx.exe") or InitiatingProcessFileName in~ ("npm.exe","node.exe","yarn.exe","pnpm.exe","npx.exe") or ProcessCommandLine has_any ("npm","yarn","pnpm")
