...
**Splunk SPL (CIM):**
```spl
``` Article-specific bespoke detection — Timing out synchronous functions with regex ```
| tstats `summariesonly` count earliest(_time) AS firstTime latest(_time) AS lastTime
    from datamodel=Endpoint.Processes
    where (Processes.process_name IN ("worker.js","benchmark.js"))
...
| tstats `summariesonly` count
    ...
    where Filesystem.action IN ("created","modified")
      AND (Filesystem.file_name IN ("worker.js","benchmark.js"))
...
| where (FileName in~ ("worker.js", "benchmark.js"))
