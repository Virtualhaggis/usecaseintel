...
The reset came directly in response to…
...
**Splunk SPL (CIM):**
```spl
``` Article-specific bespoke detection — Kimwolf DDoS Botnet Operator Arrested in Canada Over DDoS-for-Hire Attacks ```
| tstats `summariesonly` count earliest(_time) AS firstTime latest(_time) AS lastTime
    from datamodel=Endpoint.Processes
    where (Processes.process_name IN ("timeago.js","canvas-nest.js"))
...
| tstats `summariesonly` count
    ...
    where Filesystem.action IN ("created","modified")
      AND (Filesystem.file_name IN ("timeago.js","canvas-nest.js"))
...
| where (FileName in~ ("timeago.js", "canvas-nest.js"))
