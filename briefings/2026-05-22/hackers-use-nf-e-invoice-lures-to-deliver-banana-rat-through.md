# [HIGH] Hackers Use NF-e Invoice Lures to Deliver Banana RAT Through Malicious Batch Files

**Source:** Cyber Security News
**Published:** 2026-05-22
**Article:** https://cybersecuritynews.com/hackers-use-nf-e-invoice-lures/
...
`UC_12_13` · phase: **delivery** · confidence: **High**
...
`UC_12_14` · phase: **install** · confidence: **High**
...
`UC_12_15` · phase: **install** · confidence: **High**
...
`UC_12_16` · phase: **c2** · confidence: **High**
...
**Splunk SPL (CIM):**
```spl
``` Article-specific bespoke detection — Hackers Use NF-e Invoice Lures to Deliver Banana RAT Through Malicious Batch Fil ```
| tstats `summariesonly` count earliest(_time) AS firstTime latest(_time) AS lastTime
    from datamodel=Endpoint.Processes
    where (Processes.process_name IN ("consultar_nf-e.bat","backdoor.ps1","trojan.ps1"))
...
| tstats `summariesonly` count
    ...
    where Filesystem.action IN ("created","modified")
      AND (Filesystem.file_name IN ("consultar_nf-e.bat","backdoor.ps1","trojan.ps1"))
...
DeviceProcessEvents
| where Timestamp > ago(30d)
| where (FileName in~ ("consultar_nf-e.bat", "backdoor.ps1", "trojan.ps1"))
