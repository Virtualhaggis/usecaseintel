...
### Install of compromised @joyfill/layouts and @joyfill/components beta versions (PolinRider)

`UC_0_12` · phase: **delivery** · confidence: **High** · AI-generated for this article

**Splunk SPL (CIM):**
```spl
| tstats `summariesonly` count min(_time) as firstTime max(_time) as lastTime from datamodel=Endpoint.Processes where (Processes.process="*@joyfill/layouts@0.1.2-2773.beta.0*" OR Processes.process="*@joyfill/components@4.0.0-rc24-2773-beta.4*" OR Processes.process="*0.1.2-2773.beta.0*" OR Processes.process="*4.0.0-rc24-2773-beta.4*") by Processes.dest Processes.user Processes.process_name Processes.process Processes.parent_process_name
...
| where ProcessCommandLine has_any ("@joyfill/layouts@0.1.2-2773.beta.0","@joyfill/components@4.0.0-rc24-2773-beta.4","0.1.2-2773.beta.0","4.0.0-rc24-2773-beta.4")
| project Timestamp, DeviceName, AccountName, FileName, ProcessCommandLine, InitiatingProcessFileName, InitiatingProcessCommandLine
| order by Timestamp desc
```

### Node.js/build process beaconing to PolinRider hard-coded C2 IPs

`UC_0_13` · phase: **c2** · confidence: **Medium** · AI-generated for this article

**Splunk SPL (CIM):**
```spl
| tstats `summariesonly` count min(_time) as firstTime max(_time) as lastTime from datamodel=Network_Traffic.All_Traffic where (All_Traffic.dest_ip="23.27.13.43" OR All_Traffic.dest_ip="166.88.134.62" OR All_Traffic.dest_ip="198.105.127.210" OR All_Traffic.dest_ip="23.27.202.27") by All_Traffic.src All_Traffic.dest_ip All_Traffic.dest_port All_Traffic.app All_Traffic.process
...
DeviceNetworkEvents
| where Timestamp > ago(30d)
| where RemoteIP in ("23.27.13.43","166.88.134.62","198.105.127.210","23.27.202.27")
...
### Node.js/Python spawning clipboard-harvest utilities (OmniStealer/DEV#POPPER)

`UC_0_14` · phase: **actions** · confidence: **Medium** · AI-generated for this article

**Splunk SPL (CIM):**
```spl
| tstats `summariesonly` count min(_time) as firstTime max(_time) as lastTime from datamodel=Endpoint.Processes where (Processes.parent_process_name IN ("node.exe","node","python.exe","pythonw.exe","python") AND ((Processes.process_name="powershell.exe" AND Processes.process="*Get-Clipboard*") OR Processes.process_name IN ("xclip","xsel","pbpaste"))) by Processes.dest Processes.user Processes.parent_process_name Processes.process_name Processes.process
| `drop_dm_object_name(Processes)`
| `security_content_ctime(firstTime)`
| `security_content_ctime(lastTime)`
```
...
DeviceProcessEvents
| where Timestamp > ago(30d)
| where InitiatingProcessFileName in~ ("node.exe","node","python.exe","pythonw.exe","python")
| where (FileName in~ ("powershell.exe","pwsh.exe") and ProcessCommandLine has "Get-Clipboard")
   or FileName in~ ("xclip","xsel","pbpaste")
| project Timestamp, DeviceName, AccountName, InitiatingProcessFileName, InitiatingProcessCommandLine, FileName, ProcessCommandLine
| order by Timestamp desc
```

### Execution of DEV#POPPER/OmniStealer joyfill payload by SHA256

`UC_0_15` · phase: **install** · confidence: **Medium** · AI-generated for this article
