...
**Article:** https://cybersecuritynews.com/seedworm-apt-abuses-signed-fortemedia-2/
...
**Splunk SPL (CIM):**
```spl
| tstats `summariesonly` count min(_time) as firstTime max(_time) as lastTime from datamodel=Endpoint.Processes where Processes.process_name="fmapp.exe" by Processes.dest Processes.user Processes.process_name Processes.parent_process_name Processes.process_path Processes.process_hash Processes.process | `drop_dm_object_name(Processes)` | where match(process_hash, "(?i)e25892603c42e34bd7ba0d8ea73be600d898cadc290e3417a82c04d6281b743b") OR NOT match(process_path, "(?i)Program Files\\Fortemedia") | `security_content_ctime(firstTime)` | `security_content_ctime(lastTime)`
```
...
| where SHA256 == "c6182fd01b14d84723e3c9d11bc0e16b34de6607ccb8334fc9bb97c1b44f0cde"
   or InitiatingProcessSHA256 == "e25892603c42e34bd7ba0d8ea73be600d898cadc290e3417a82c04d6281b743b"
   or InitiatingProcessFolderPath !contains "Program Files"
| project Timestamp, DeviceName, InitiatingProcessAccountName,
          InitiatingProcessFolderPath, FolderPath, FileName, SHA256, InitiatingProcessSHA256
...
**Splunk SPL (CIM):**
```spl
| tstats `summariesonly` count min(_time) as firstTime max(_time) as lastTime from datamodel=Endpoint.Processes where Processes.process_name="sentinelmemoryscanner.exe" by Processes.dest Processes.user Processes.process_name Processes.parent_process_name Processes.process_path Processes.process_hash | `drop_dm_object_name(Processes)` | where match(process_hash, "(?i)128b58a2a2f1df66c474094aacb7e50189025fbf45d7cd8e0834e93a8fbed667") OR NOT match(process_path, "(?i)Sentinel") | `security_content_ctime(firstTime)` | `security_content_ctime(lastTime)`
```
...
| where SHA256 == "0c9b911935a3705b0ad569446804d80026feb6db3884aeb240b6c76e9b8cf139"
   or InitiatingProcessSHA256 == "128b58a2a2f1df66c474094aacb7e50189025fbf45d7cd8e0834e93a8fbed667"
   or InitiatingProcessFolderPath !contains "Sentinel"
| project Timestamp, DeviceName, InitiatingProcessAccountName,
          InitiatingProcessFolderPath, FolderPath, FileName, SHA256, InitiatingProcessSHA256
| order by Timestamp desc
```
...
| where ProcessCommandLine has ".xml"
   or InitiatingProcessFileName in~ ("wscript.exe","cscript.exe","mshta.exe","cmd.exe","powershell.exe","explorer.exe")
...
**Splunk SPL (CIM):**
```spl
| tstats `summariesonly` count min(_time) as firstTime max(_time) as lastTime from datamodel=Network_Traffic.All_Traffic where All_Traffic.dest_ip="179.43.177.220" All_Traffic.dest_port=8080 by All_Traffic.src All_Traffic.user All_Traffic.app All_Traffic.dest_ip All_Traffic.dest_port All_Traffic.url | `drop_dm_object_name(All_Traffic)` | `security_content_ctime(firstTime)` | `security_content_ctime(lastTime)`
```
...
| where InitiatingProcessFileName in~ ("powershell.exe", "pwsh.exe", "curl.exe", "cmd.exe", "node.exe", "bitsadmin.exe", "certutil.exe")
...
**Splunk SPL (CIM):**
```spl
| tstats `summariesonly` count min(_time) as firstTime max(_time) as lastTime from datamodel=Endpoint.Registry where (Registry.registry_path="*\\CurrentVersion\\Run\\*" OR Registry.registry_path="*\\CurrentVersion\\RunOnce\\*") (Registry.registry_value_data="*node.exe*" OR Registry.registry_value_data="*fmapp.exe*" OR Registry.registry_value_data="*sentinelmemoryscanner.exe*" OR Registry.registry_value_data="*sentinelagentcore.dll*" OR Registry.registry_value_data="*fmapp.dll*" OR Registry.registry_value_data="*.xml*") by Registry.dest Registry.user Registry.registry_path Registry.registry_value_name Registry.registry_value_data Registry.process_name | `drop_dm_object_name(Registry)` | `security_content_ctime(firstTime)` | `security_content_ctime(lastTime)`
```
...
| where RegistryValueData has_any ("node.exe", "fmapp.exe", "sentinelmemoryscanner.exe", "fmapp.dll", "sentinelagentcore.dll", ".xml")
| project Timestamp, DeviceName, InitiatingProcessAccountName,
...
**Splunk SPL (CIM):**
```spl
| tstats `summariesonly` count min(_time) as firstTime max(_time) as lastTime from datamodel=Network_Traffic.All_Traffic where (All_Traffic.dest="sendit.sh" OR All_Traffic.dest="*.sendit.sh" OR All_Traffic.dest="svc.wompworthy.com" OR All_Traffic.dest="timetrakr.cloud" OR All_Traffic.dest_ip IN ("178.128.233.36","172.67.156.47","104.21.48.205","37.187.78.41","34.117.59.81")) by All_Traffic.src All_Traffic.user All_Traffic.app All_Traffic.dest All_Traffic.dest_ip All_Traffic.url All_Traffic.bytes_out | `drop_dm_object_name(All_Traffic)` | `security_content_ctime(firstTime)` | `security_content_ctime(lastTime)`
```
...
DeviceNetworkEvents
| where Timestamp > ago(30d)
| where RemoteUrl has_any ("sendit.sh", "svc.wompworthy.com", "wompworthy.com", "timetrakr.cloud")
   or RemoteIP in ("178.128.233.36", "172.67.156.47", "104.21.48.205", "37.187.78.41", "34.117.59.81")
| project Timestamp, DeviceName, RemoteIP, RemotePort, RemoteUrl,
          InitiatingProcessFileName, InitiatingProcessCommandLine,
