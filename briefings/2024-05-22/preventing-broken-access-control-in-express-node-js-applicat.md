...
**Splunk SPL (CIM):**
```spl
``` Article-specific bespoke detection — Preventing broken access control in express Node.js applications ```
| tstats `summariesonly` count earliest(_time) AS firstTime latest(_time) AS lastTime
    from datamodel=Endpoint.Processes
    where (Processes.process_name IN ("node.js","passport.js"))
...
| tstats `summariesonly` count
    ...
    where Filesystem.action IN ("created","modified")
      AND (Filesystem.file_name IN ("node.js","passport.js"))
...
| where (FileName in~ ("node.js", "passport.js"))
