...
### CornFlake RAT implant dropped/executed as svchost32.exe in %APPDATA%\svchost32

`UC_1_9` · phase: **install** · confidence: **High** · AI-generated for this article

**Splunk SPL (CIM):**
```spl
| tstats `summariesonly` count min(_time) as firstTime max(_time) as lastTime from datamodel=Endpoint.Filesystem where Filesystem.file_name="svchost32.exe" AND Filesystem.file_path="*\\AppData\\Roaming\\svchost32\\*" by Filesystem.dest Filesystem.file_path Filesystem.file_name Filesystem.process_id | `drop_dm_object_name(Filesystem)` | convert ctime(firstTime) ctime(lastTime)
```
...
### CornFlake persistence: 'Cloud Sync Service' (svchost32) service install + Run key
...
(DeviceEvents
 ...
 | where AdditionalFields has "Cloud Sync Service" or AdditionalFields has "svchost32"
 | project Timestamp, DeviceName, Signal="ServiceInstalled", Detail=AdditionalFields,
           InitiatingProcessFileName, InitiatingProcessCommandLine, InitiatingProcessAccountName),
(DeviceRegistryEvents
 | where Timestamp > ago(30d)
 | where RegistryKey has @"\CurrentVersion\Run"
 | where RegistryValueData has "svchost32"
 | project Timestamp, DeviceName, Signal="RunKey", Detail=strcat(RegistryValueName, " = ", RegistryValueData),
           InitiatingProcessFileName, InitiatingProcessCommandLine, InitiatingProcessAccountName)
| order by Timestamp desc
```

### ChocoShell token theft from Token Broker cache (.tbres / TokenBroker\Cache)

`UC_1_11` · phase: **actions** · confidence: **Medium** · AI-generated for this article

**Splunk SPL (CIM):**
```spl
| tstats `summariesonly` count min(_time) as firstTime max(_time) as lastTime from datamodel=Endpoint.Processes where (Processes.process_name IN ("powershell.exe","pwsh.exe") OR Processes.parent_process_name IN ("powershell.exe","pwsh.exe")) AND (Processes.process="*TokenBroker*" OR Processes.process="*.tbres*") by Processes.dest Processes.user Processes.process_name Processes.process Processes.parent_process_name | `drop_dm_object_name(Processes)` | convert ctime(firstTime) ctime(lastTime)
```
...
| where FileName in~ ("powershell.exe","pwsh.exe","cmd.exe") or InitiatingProcessFileName in~ ("powershell.exe","pwsh.exe")
...
### Entra device-code / Authentication Broker sign-in abuse (CaptiveCrunch phishing)

`UC_1_12` · phase: **delivery** · confidence: **Medium** · AI-generated for this article
...
| summarize SignIns=count(), Countries=make_set(Country), IPs=make_set(IPAddress), FirstSeen=min(Timestamp), LastSeen=max(Timestamp) by AccountUpn, Application
| where array_length(Countries) >= 1
| order by LastSeen desc
```

### Egress to CaptiveCrunch C2 / Microsoft-impersonating domains and IPs

`UC_1_13` · phase: **c2** · confidence: **Medium** · AI-generated for this article
...
| project Timestamp, DeviceName, InitiatingProcessAccountName, InitiatingProcessFileName,
          InitiatingProcessCommandLine, RemoteIP, RemoteUrl, RemotePort, InitiatingProcessFolderPath
| order by Timestamp desc
```
