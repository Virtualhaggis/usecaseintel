# [HIGH] Hackers Hijack Microsoft Teams Accounts to Deliver ModeloRAT

**Source:** Cyber Security News
**Published:** 2026-05-12
**Article:** https://cybersecuritynews.com/hackers-hijack-microsoft-teams/
...
**Splunk SPL (CIM):**
```spl
``` Article-specific bespoke detection — Hackers Hijack Microsoft Teams Accounts to Deliver ModeloRAT ```
| tstats `summariesonly` count earliest(_time) AS firstTime latest(_time) AS lastTime
    from datamodel=Endpoint.Processes
    where (Processes.process_name IN ("pythonw.exe"))
...
| tstats `summariesonly` count
    ...
    where Filesystem.action IN ("created","modified")
      AND (Filesystem.file_name IN ("pythonw.exe"))
